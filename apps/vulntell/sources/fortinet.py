"""VulnTell Fortinet PSIRT 适配器（阶段 8，Task 8.3）。

将 Fortinet PSIRT API (https://www.fortinet.com) 响应映射为 SourceRecord。
支持分页、游标、错误分类和 FortiOS 产品漏洞公告。
...
class FortinetConfig:
    """Fortinet PSIRT 配置。"""
    endpoint: str = "https://www.fortinet.com/fortiguard/psirt"
    timeout_seconds: float = 30.0
    page_size: int = 100


class FortinetAdapter:
    """Fortinet PSIRT 适配器：将 Fortinet API 映射为 SourceRecord 协议。
    ...
    def __init__(
        self,
        config: Optional[FortinetConfig] = None,
        transport: Optional[Callable] = None,
    ) -> None:
        self._config = config or FortinetConfig()
    ...
    async def fetch_page(
        ...
        # 添加过滤条件
        if request.filters.get("fortiguard_id"):
            params["fortiguard_id"] = request.filters["fortiguard_id"]
        ...
        try:
            ...
            return SourceError.from_exception("fortinet", exc)
        ...
        try:
            ...
            for adv in advisories:
                fortiguard_id = adv.get("fortiguard_id", "")
                if not fortiguard_id:
                    continue
                ...
                record = SourceRecord(
                    source_record_id=fortiguard_id,
                    payload={
                        "id": fortiguard_id,
                        "title": title,
                        "summary": summary,
    ...
    async def _default_transport(
        ...
        raise NotImplementedError(
            "Fortinet adapter requires a transport function. "
            "Use httpx or aiohttp transport for live mode."
        )


class FortinetTransport:
    """Fortinet PSIRT HTTP transport（使用 httpx）。"""
