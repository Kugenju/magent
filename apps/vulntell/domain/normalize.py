"""确定性标准化与质量问题（阶段 2 从 examples.vulntell.normalize 迁移，纯规则不变）。
...
from .policies import is_cve, is_cwe, is_url
...
def normalize_record(raw: RawSourceRecord) -> tuple[SourceObservation, list[QualityIssue]]:
    ...
    cve_id = p.get("cve_id")
    if cve_id is not None and not is_cve(cve_id):
    ...
    obs = SourceObservation(
        source=src,
        source_record_id=rid,
        cve_id=cve_id if is_cve(cve_id) else None,
