"""VulnTell EUVD 适配器（阶段 6，Task 6.3）。

将 EUVD API (https://europeanswift.org/api) 响应映射为 SourceRecord。
支持分页、游标、错误分类和欧盟协调披露信息。
...
class EUVDConfig:
    """EUVD 配置。"""
    endpoint: str = "https://europeanswift.org/api"
    timeout_seconds: float = 30.0
    page_size: int = 100  # EUVD 每页记录数


class EUVDAdapter:
    """EUVD 适配器：将 EUVD API 映射为 SourceRecord 协议。
    ...
    def __init__(
        self,
        config: Optional[EUVDConfig] = None,
        transport: Optional[Callable] = None,
    ) -> None:
        self._config = config or EUVDConfig()
    ...
    async def fetch_page(
        ...
        if request.filters.get("vendor"):
            params["vendor"] = request.filters["vendor"]
        if request.filters.get("product"):
            params["product"] = request.filters["product"]
        ...
        try:
            ...
            return SourceError.from_exception("euvd", exc)
        ...
        try:
            ...
            for vuln in vulnerabilities:
                ...
                risk_rating = vuln.get("risk_rating")
                cvss_score = vuln.get("cvss_score")
                cvss_vector = vuln.get("cvss_vector")

                # 提取别名
                aliases = []
                if vuln.get("enisa_id"):
                    aliases.append(vuln["enisa_id"])
    ...
    async def _default_transport(
        ...
        raise NotImplementedError(
            "EUVD adapter requires a transport function. "
            "Use httpx or aiohttp transport for live mode."
        )


class EUVDTransport:
    """EUVD HTTP transport（使用 httpx）。"""
