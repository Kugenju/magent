"""加载冻结的离线 fixture 与数据集元数据（阶段 2 从 examples.vulntell.loading 迁移）。

加载器是确定性的：同一文件始终得到相同 ``RawSourceRecord``（payload hash 用
canonical_json 计算，与字段顺序无关）。默认应用只使用 fixture，从不联网。

fixture 路径由应用配置集中解析（见 apps.vulntell.config / 应用 FIXTURE_DIR），
不依赖当前工作目录。
"""
